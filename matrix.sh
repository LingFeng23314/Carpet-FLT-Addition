...
for path in "${paths[@]}"; do
  ...
  for folder in $folders; do
    ...
    if [[ "$folderName" != "origin" ]]; then
      ...
      # 26.x 是独立 gradle 项目（非 ModMultiVersion 模板生成，JDK 25、不跑 init.sh）：
      # 版本号以 "26." 开头即标记 independent=true，build.yml 据此走独立构建流程
      independent="false"
      if [[ "$mcVersion" == 26.* ]]; then
        independent="true"
      fi
      allFolderObjects+=("{\"mc-version\": \"$mcVersion\", \"mc-loader\": \"$mcLoader\", \"publish-loaders\": \"$publishLoaders\", \"publish-version\": \"$supportVersion\", \"independent\": \"$independent\"}")
    fi
  done
done
